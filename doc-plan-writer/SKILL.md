...
方案文档执行时**只能修改设计文档**（L2/L3/L4 docs/），**不能直接写代码**：
...
**方案→设计**：
1. 方案审批状态为"✅已批准"
2. 执行步骤 = 更新设计文档（表结构、接口定义、架构图等）
3. ❌ **禁止**在方案执行阶段直接写代码
4. ✅ 设计文档使用项目定义的模板
5. **设计原则迁移**：
   - 计划文档中的 `CONFIRMED` 原则迁移到设计文档
   - 在设计文档中保留 `<!-- CONFIRMED: 日期 要点 -->` 标注
   - 原则内容放在设计文档的"设计原则"或"约束条件"章节
   - 设计文档中的原则是最终权威版本（唯一真相源）

**方案文档写作规范**：
- ❌ 不要写"原设计"、"旧方案"等迭代过程描述
- ❌ 不要在方案中写具体代码实现
- ✅ 聚焦于需求和约束条件
- ✅ 设计对比放在"设计决策"章节，说明选择理由
- ✅ 执行步骤应是"更新XX设计文档"而非"实现XX代码"
