...
| 所有 kind | 已通过区均含 `subtype` 列（prefix 之后） |
...
- article 须获取正文（仅标题/摘要不算），filing/calendar/metric 须获取结构化数据
- 一端点一行：同一 provider 的不同端点必须拆为独立行
- 分配 type 后须在 `samples/feed/raw/{kind}/` 下保留样本
...
- feedList_id/name/prefix 在全部 4 个 kind 子文档中不重复（含废弃区，废弃标识永不复用）
