...
> 流程：前置知识 → 发现与注册 → 协议探测 → 完整度评估 → feed_enrich → 选择器发现 → 字段推断 → 迁移输出 → 进化
...
|------|-----------|--------|---------|
| `article` | 1 | 10001-19999 | **人工编辑或 UGC 文本内容**（媒体文章+社交帖子） |
| `filing` | 2 | 20001-29999 | **政府/监管/国际组织/评级机构**的官方文本发布 |
| `calendar` | 3 | 30001-39999 | 仅**日期**信息，不含数据内容 |
| `metric` | 4 | 40001-49999 | 输出为 **(subject, subtype, date, value)** 的数值型数据 |

**判定顺序**：政府/国际组织/监管/评级机构 → 一律 `filing`；数值型时序数据 → `metric`；仅日期事件 → `calendar`；其余文本内容（媒体+UGC）→ `article`

> **probe_source.py 评估模式映射**：`article` kind 在探测脚本中拆分为 `--kind news`（媒体，正文要求严格）和 `--kind social`（UGC，正文要求宽松）两种评估模式，对应不同的 gates/signals 标准。
...
2. **分配 feedList_id**：读取目标 kind 文档（`docs/Feed/storage/feedList/{kind}.md`），扫描全文所有已用 ID，在该 kind 的 ID 范围内（如 article: 10001-19999）找到最小未使用 ID
3. **生成标识**：
...
metric kind（多 subtype 列）：
```
| feedList_id | name | prefix | subtype | name_cn | priority | note |
```
...
| `summary` | desc ≥100字符 + 含实体/事件信号 + feed_enrich 不可行 + **priority ≥ 10** | ✅ 迁移 | **feed_only**（仅消费摘要） |
| `minimal` | desc <100字符 或无 desc 或 priority < 10 的纯摘要 | ❌ 未通过 | — |

**summary 级迁移条件**（全部满足）：
1. RSS 可达（HTTP 200 + ≥1 item + pubDate ≤90天）
2. description 平均 ≥100 字符（含核心事件+实体信号）
3. feed_enrich 已测试且不可行（Cloudflare/paywall/无正文）
4. priority ≥ 10（信源权威性足以补偿内容深度不足）
...
| kind | mode | threshold | 信号体系 |
|------|------|-----------|---------|
| news | web | **0.70** | **多维信号矩阵**（§4.4.1）：结构(×3)+语义(×3)+反截断(×2)+字符辅助(×1)，feed_enrich 时加增益比(×2) |
| news | rss | 0.45 | 传统：body_richness(3.0), data_freshness(2.0), structural_depth(2.0), no_truncation(2.0), item_consistency(1.0) |
| news | api | 0.40 | 传统：body_richness(3.0), field_quality(1.0) |
| filing | web | **0.70** | **多维信号矩阵**（§4.4.1） |
| filing | rss | 0.30 | 传统：body_richness(2.0), data_freshness(2.0), no_truncation(1.0) |
| filing | api | 0.45 | 传统：field_coverage(3.0), field_quality(2.0), has_date(1.0) |
| calendar | web | 0.30 | 传统：body_richness(1.5), has_title(1.0) |
| calendar | rss | 0.20 | 传统：data_freshness(2.0), body_richness(0.5) |
| calendar | api | 0.40 | 传统：field_coverage(3.0), field_quality(1.0) |
| social | web | 0.25 | 传统：body_richness(2.0) |
| social | rss | 0.30 | 传统：body_richness(2.0), data_freshness(2.0), item_count(0.5) |
| social | api | 0.30 | 传统：body_richness(2.0), field_quality(1.0), item_count(0.5) |
| metric | api | 0.40 | 传统：field_coverage(3.0), field_quality(1.0) |
...
**适用场景**：article kind 中 80%+ 的 RSS 源仅有摘要。
...
Step 5: 迁移（见 §8.6 迁移检查模板）
  ...
  ↓ 全部完成
Step 6: YAML 配置输出
  crawl_mode=feed_enrich, crawl_content_selector={sel}
```
...
```bash
cp /tmp/probe_{type}.{ext} samples/feed/raw/{kind}/{feedList_id}_{name}_{type}_{YYYYMMDD}.{ext}
```
...
□ 1. 在目标 type 区正确位置插入新行
      → 按 priority DESC, feedList_id ASC 排序
      → status = designed
      → note 包含所有隐藏字段编码
...
**标识唯一性**：
- [ ] feedList_id 在该 kind 范围内且跨 4 个 kind 唯一（含废弃区，废弃标识不可复用）
...
- [ ] `kind=news` 时，`content_level=full` 或 `content_level=summary`（p≥10 + desc≥100字符）可迁移；`minimal` 保留未通过
...
**样本留档**：
- [ ] 样本已保存到 `samples/feed/raw/{kind}/` 并格式化
