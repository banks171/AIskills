...
- ❌ **严禁加入任何 `style` 背景样式**（例如：style Layer1 fill:#e3f2fd）⚠️ Claude 易犯
- ✅ 图表不应过宽或过长，影响可读性
- ✅ 连线需标明语义 label
...
**用途**：表述技术实现时使用Python关键字配合中文变量

```python
# 表述技术实现时使用Python关键字配合中文变量
用户信息 = 获取注册信息()
if 手机号/邮箱未重复且格式正确:
    发送验证码()
    if 验证码验证通过:
...
**规范**：
- ✅ 关键字用Python：`if/else`, `for`, `return`, `and/or`
- ✅ 变量用中文：`用户信息`, `手机号`, `验证码`
- ✅ 流程用文字+符号：`保存 → 返回`, `A / B`, `>`, `==`
- ❌ 不用编号：~~`1) 若xxx 2) 则xxx`~~
...
- [ ] **Mermaid style**：搜索 `style.*fill:`，必须为0结果
