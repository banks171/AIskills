...
**多子图布局防挤规范** ⚠️ Claude 易犯：

多个独立子图（subgraph）并排时容易挤成一团，需根据子图数量选择布局方向：

| 子图数量 | 推荐布局 | 说明 |
|---------|---------|------|
| ≤3 | `graph TB`（默认纵向） | 子图横向自然排列，不会太挤 |
| 4-5 | `graph LR` + `direction TB` | 外层左右排列，内部纵向展开 |
| >5 | 拆分为多张图 | 单张图无法清晰展示 |

````markdown
✅ **正确**：4-5 个子图用 `graph LR` + 内部 `direction TB`
```mermaid
graph LR
    subgraph R1["路线一"]
        direction TB
...
❌ **错误**：多子图用 `graph TB`，所有子图挤在同一行
```mermaid
graph TB
    subgraph R1["路线一：图快照序列方法"]
        A1["R-GCN 编码空间结构"] --> A2["GRU 捕获时序"] --> A3["解码器预测"]
    end
    subgraph R2["路线二：双通道融合方法"]
        B1["GNN 结构编码"] --> B2["时序注意力"] --> B3["注意力融合输出"]
    end
```
````

**关键规则**：
- ✅ 子图间用 `~~~`（隐形边）控制排列顺序：`R1 ~~~ R2 ~~~ R3`
- ✅ 子图标题简短：≤6汉字（如 `"路线一"` 而非 `"路线一：图快照序列方法"`）
- ✅ 子图内节点标签精简：≤4汉字（如 `"编码"` 而非 `"R-GCN 编码空间结构"`）
- ❌ 不要在 `graph TB` 中放 4+ 个并列子图（会横向溢出）
- ❌ 不要在子图末尾节点间连 `~~~`（会产生阶梯错位），应在子图 ID 间连
...
- [ ] **Mermaid 子图拥挤**：`graph TB` 中并列 subgraph ≥4 时，改为 `graph LR` + `direction TB`
